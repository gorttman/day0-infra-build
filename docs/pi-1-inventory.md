...
**Correction (2026-07-01):** this fix was originally recorded as "baked into `roles/prep_rootfs/tasks/configure_rsyslog.yml` for future rebuilds," but that role built to an orphaned staging path never synced to the live `nfs_os_path` (`/srv/nfs/rpios/latest`) — see `docs/rebuild-gap-audit.md` item 4. That role has since been deleted as dead code.  
**Now automated:** `roles/nfs_netboot/tasks/install_rsyslog_base.yml`, called from `configure_nfs_root_common.yml` during `--tags manage_nfs`, chroot-installs rsyslog and writes the forwarding config directly into `nfs_os_path` — see `docs/rebuild-gap-audit.md` item 4 follow-up.
...
| rsyslog config in rootfs | `roles/nfs_netboot/tasks/install_rsyslog_base.yml` — chroot-installs rsyslog + writes forwarding config into the base rootfs (see 12a correction above) |
...
**Fix (2026-06-03):** Wrote `nameserver 1.1.1.1` + `nameserver 8.8.8.8` to the per-node overlay resolv.conf. Note: pihole at `192.168.2.1` is unreachable from pinode-01 (same WiFi AP isolation), so upstream DNS is used directly.  
**Regression (found 2026-07-03):** Because `/etc/resolv.conf` is `# Generated by NetworkManager`, NetworkManager rewrites it on every start/reconnect — the static nameserver lines from the fix above don't survive. At some point a `[global-dns-domain-*]` block was hand-added to the overlay's `NetworkManager.conf` to make the override durable, but it was missing its required `[global-dns]` parent section, so NetworkManager only honored it inconsistently (empty `resolv.conf` was observed after a node reboot, which also blocked NTP sync and caused a cascading ~8.5h `NotReady` outage — clock skew broke k3s-agent's TLS validation against the server).  
**Now automated:** `roles/nfs_netboot/tasks/setup_dns_overlay.yml` (called from `add_node.yml` via `--tags manage_nodes`) writes a correctly-formed `NetworkManager.conf` with both `[global-dns]` and `[global-dns-domain-*]` sections into each node's `/etc` overlay. Applied directly to pinode-01's live overlay on 2026-07-03 pending its next `manage_nodes` run.
...
The home WiFi AP has client isolation enabled. Neither node can reach the other's WiFi IP (`192.168.2.10` ↔ `192.168.2.11`). All inter-node traffic that requires direct reachability (k3s tunnel, NFS, syslog) must use the wired management network (`192.168.1.x`).

**Correction (2026-08-10):** the line above about flannel "tunneling through the k3s WebSocket" is wrong and should not be trusted — flannel's VXLAN data plane is a direct UDP overlay between nodes' own registered node-ips, entirely independent of the `exec`/`logs` WebSocket tunnel described in Fix 13b. Confirmed live: pod-to-pod traffic between k8smaster and pinode-01 worked correctly with `nc -zv` tests even *before* Fix 13e below (i.e., while k8smaster's node-ip was still auto-detected onto `192.168.2.10`/wlan0). The actual reason it worked is simpler and consistent with the isolation note above it: AP client isolation blocks WiFi-client-to-WiFi-client traffic specifically, not wired-to-WiFi traffic — and pinode-01's node-ip has always been wired (Fix 13a), so every cross-node conversation had at least one wired leg regardless of what k8smaster's own node-ip was.

**13e — k8smaster's own node-ip/flannel endpoint also auto-detected onto wlan0 (found 2026-08-09/10)**  
Fix 13a pinned *pinode-01's* node-ip to wired back in June, but k8smaster's own k3s **server** never got the equivalent `node-ip` setting — only `advertise-address`/`tls-san` (Fix 13b), which only affects the exec/logs tunnel, not the node's own registered identity or flannel's VXLAN endpoint for *its own* pods. k8smaster's `flannel.alpha.coreos.com/public-ip` and `status.addresses` sat on `192.168.2.10` (wlan0) for as long as this cluster has existed. This surfaced during an unrelated PIA VPN outage investigation that turned into a full networking audit of both nodes — see the homelab-book chapter for the full narrative of how many wrong turns it took to get here.  
**Fix:** Added `node-ip: 192.168.1.10` to the same Ansible-managed block in `/etc/rancher/k3s/config.yaml` as Fix 13b, restarted k3s. `status.addresses` updated correctly, but **`flannel.alpha.coreos.com/public-ip` did not self-correct** — it kept the stale `192.168.2.10` value until manually overwritten with `kubectl annotate node k8smaster flannel.alpha.coreos.com/public-ip=192.168.1.10 --overwrite`. This looks like general k3s/flannel behavior (the same staleness was independently found on pinode-01's annotation the same night, from an unrelated cause), not a one-off — any future node-ip change on either node needs this same manual annotate step.  
**Now automated:** `roles/install_required_software/tasks/post_install.yml` now sets `node-ip` in the same block as Fix 13b, waits for the API to come back after the restart, and runs the `flannel.alpha.coreos.com/public-ip` annotate command as an explicit task.  
**Verified:** cross-node pod traffic (`nc -zv`) working both directions after the change, on the identical wired-to-wired path Fix 13a already established for the reverse direction.
